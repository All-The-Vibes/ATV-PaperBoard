...
### Added
- **Agent-first `/paperboard` skill.** Users can now stay inside their preferred agent and run `/paperboard path/to/file.md`, `/paperboard path/to/proposal.md --style meridian`, `/paperboard styles`, or `/paperboard gallery`. The skill maps those commands to the local CLI under the hood, so the CLI remains the engine while the human-facing workflow is agent-native.
- **Named style presets.** `paperboard render` now supports `--style paperboard|meridian|atv`, plus `paperboard styles list` and `paperboard styles show <style>`. `paperboard` is the default style; `meridian` is opt-in for proposals and decision memos; `atv` is opt-in for dense technical reports.
- **Meridian proposal style.** Added `core/designs/meridian.DESIGN.md`, a dark editorial design system for authored proposals, plans, reviews, and decision docs.
- **Proposal section primitives.** Added `decision-card`, `option-cards`, and `file-tree` section kinds for decision-first artifacts. Markdown tables and typed tables now carry `data-label` values for mobile definition-row rendering.
- **`paperboard schema` subcommand** — agent-discoverable documentation of the atv-tier section graph. Without args, lists all 18 kinds (`hero`, `sec`, `stack-list`, `dep-list`, `q-list`, `steps`, `decision-card`, `option-cards`, `file-tree`, `code-shell`, `color-strip`, `fit-row`, `anti`, `checklist`, `callout`, `subhead`, `props-table`, `status-table`) with one-line descriptions. `--kind <name>` shows fields + an example JSON payload for one kind. `--format json` emits a machine-readable dump. `--list-kinds` prints kind names one per line for scripting. Closes the discoverability gap that meant agents installed via the skill defaulted to prose/table output and never exercised the card-tier rendering path.
...
### Changed
- **Default style is PaperBoard.** Omitting `--style` resolves to `paperboard.DESIGN.md`; Meridian and ATV are selectable style presets. Raw `--design <name|path|url>` remains the advanced escape hatch and overrides style selection.
- **README now leads with agent-first usage.** The standalone CLI section is framed as the scripting/CI/debugging path, while the main workflow is installing PaperBoard in an agent and invoking `/paperboard`.
